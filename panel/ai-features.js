...
window.renderLLMResponse = function (options) {
  const {
    ...
    tokens = 0,
    readingTime = '',
    disclaimer = 'This response was generated by AI. Always verify information independently before making decisions.'
  } = options;
...
window.downloadFile = function (options) {
  const { content, filename, format, title = 'Export' } = options;
...
<body>
  ...
  <footer style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee; font-size: 12px; color: #999;">
    Exported from SF Pro Toolkit on ${new Date().toLocaleString()}
  </footer>
</body>
</html>`;
  ...
  try {
    ...
    if (window.showToast) window.showToast(`Downloaded as ${extension.toUpperCase()} ✓`, 'success');
  } catch (error) {
...
window.regenerateDiagnosticsWithAI = async function () {
  ...
  if (tab && tab.url) {
    ...
    if (!isSAPDomain) {
      if (window.showToast) {
        window.showToast('ℹ️ AI Diagnostics only available on SAP domains', 'info');
        setTimeout(() => {
          if (window.showToast) window.showToast('Navigate to a SAP page (SuccessFactors, S/4HANA, BTP, or sap.com) to use this feature', 'info');
        }, 2000);
      }
      return;
    }
  ...
  try {
    ...
    contentDiv.innerHTML = window.renderLLMResponse({
      ...
      disclaimer: 'This response was generated by AI analyzing your page. While AI provides helpful insights, always verify information independently before making decisions.'
    });
...
function displayEnhancedAISearchResults(query, response, context) {
  ...
  aiInsightsContent.innerHTML = `<div style="padding: 12px;">${window.renderLLMResponse({
    ...
    disclaimer: 'This response was generated by AI analyzing your toolkit data. While AI provides helpful insights, always verify information independently before making decisions.'
  })}</div>`;
...
function showEstimateResults(result) {
  ...
  contentEl.innerHTML = window.renderLLMResponse({
    ...
    disclaimer: 'This response was generated by AI. While AI provides helpful insights, always verify information independently before making decisions.'
  });
...
${costLine}Generated: ${timestamp}`;
    ...
    if (recentDuplicate) {
      if (window.showToast) {
        window.showToast('⚠️ This AI response was already saved recently', 'warning');
      }
...
window.addShortcutWithAI = async function () {
  ...
  try {
    ...
    if (!isSAPDomain) {
      ...
      notesField.style.borderColor = '';
      if (window.showToast) {
        window.showToast('ℹ️ AI Shortcut creation only available on SAP domains', 'info');
        setTimeout(() => {
          if (window.showToast) window.showToast('Navigate to a SAP page (SuccessFactors, S/4HANA, BTP, or sap.com) to use this feature', 'info');
        }, 2000);
      }
      // Still populate basic fields manually
      document.getElementById('shortcutName').value = tab.title;
      document.getElementById('shortcutPath').value = tab.url;
      return;
    }
