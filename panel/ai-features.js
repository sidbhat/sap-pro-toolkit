...
window.renderLLMResponse = function (options) {
  ...
  // Metadata badges (optional)
  const badgesHTML = showBadges ? `
    <div class="llm-response-badges">
      ${model ? `<span class="llm-response-badge model">🤖 ${model}</span>` : ''}
      ${tokens ? `<span class="llm-response-badge tokens">📊 ${tokens.toLocaleString()} tokens</span>` : ''}
      ${readingTime ? `<span class="llm-response-badge time">⏱️ ${readingTime}</span>` : ''}
    </div>
  ` : '';
  ...
  return `
    ...
    <div class="llm-response-card">
      <div class="llm-response-content ai-response">
        ${formattedContent}
      </div>
      <div class="llm-response-disclaimer">
        <strong>✨ AI INSIGHTS</strong> – ${disclaimer}
      </div>
    </div>
  `;
};
...
window.regenerateDiagnosticsWithAI = async function () {
  ...
  try {
    ...
    contentDiv.innerHTML = window.renderLLMResponse({
      ...
      contextHeader: `<strong>📄 ${scrapedData.title || tab.title}</strong>`,
      disclaimer: 'This response was generated by AI analyzing your page. Always verify information independently before making decisions.'
    });
...
window.handleRunAIPrompt = async function () {
  ...
  // Apply inline loading state (same pattern as AI Diagnostics and AI Search)
