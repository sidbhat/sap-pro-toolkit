...
window.regenerateDiagnosticsWithAI = async function () {
  ...
  try {
    ...
    // ✅ CLEAN REPORT LAYOUT - Content first, metadata in footer
    contentDiv.innerHTML = `
      ...
      <div style="margin-top: 16px; padding-top: 12px; border-top: 1px solid var(--border); font-size: 10px; color: var(--text-secondary); opacity: 0.7;">
        <strong>📋 Report:</strong> ${scrapedData.title || tab.title}<br>
        <strong>🤖 Model:</strong> ${result.model || 'AI'} · ${(result.usage?.inputTokens || 0) + (result.usage?.outputTokens || 0)} tokens
      </div>
    `;
