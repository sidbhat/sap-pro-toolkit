// SAP Pro Toolkit - AI Features Module
// Heavy AI logic: diagnostics, search, cost estimator, prompts

// ==================== SHARED LLM RESPONSE RENDERER ====================

/**
 * Unified LLM response renderer for consistent display across all AI features.
 * @param {Object} options - Rendering options
 * @param {string} options.content - Raw AI response content (Markdown)
 * @param {string} [options.contextHeader] - Optional context header HTML (e.g., query info, page title)
 * @param {boolean} [options.showBadges=false] - Whether to show metadata badges
 * @param {string} [options.model] - Model name for badge
 * @param {number} [options.tokens] - Total tokens for badge
 * @param {string} [options.readingTime] - Reading time for badge
 * @param {string} [options.disclaimer] - Custom disclaimer text (default provided)
 * @returns {string} - HTML string for the LLM response card
 */
window.renderLLMResponse = function (options) {
  const {
    content,
    contextHeader = '',
    showBadges = false,
    ...
    tokens = 0,
    readingTime = '',
    disclaimer = window.i18n('aiDisclaimerGeneric')
  } = options;

  // Context header (optional)
  const contextHTML = contextHeader ? `
    <div class="llm-response-context-header">
      ${contextHeader}
    </div>
  ` : '';

  // Metadata badges (optional) - inline to prevent whitespace issues
  const badgesHTML = showBadges ? `<div class="llm-response-badges">${model ? `<span class="llm-response-badge model">🤖 ${model}</span>` : ''}${tokens ? `<span class="llm-response-badge tokens">📊 ${tokens.toLocaleString()} tokens</span>` : ''}${readingTime ? `<span class="llm-response-badge time">⏱️ ${readingTime}</span>` : ''}</div>` : '';

  // Main response card
  const formattedContent = typeof markdownToHTML === 'function' ? markdownToHTML(content) : content;

  return `${contextHTML}${badgesHTML}<div class="llm-response-card"><div class="llm-response-content ai-response">${formattedContent}</div><div class="llm-response-disclaimer"><strong>✨ AI INSIGHTS</strong> – ${disclaimer}</div></div>`;
};

// ==================== SHARED DOWNLOAD UTILITY ====================

/**
 * Download content in specified format (txt, md, html).
 * @param {Object} options - Download options
 * @param {string} options.content - The content to download (raw markdown or text)
 * @param {string} options.filename - Base filename without extension
 * @param {string} options.format - 'txt', 'md', or 'html'
 * @param {string} [options.title] - Optional title for HTML wrapper
 */
window.downloadFile = function (options) {
  const { content, filename, format, title = 'Export', headerDisclaimer = '' } = options;
  ...
  switch (format) {
    case 'html':
      ...
      const htmlContent = typeof markdownToHTML === 'function' ? markdownToHTML(content) : content;
      const disclaimerHTML = headerDisclaimer ?
        `<div style="background: #FFF4E5; border: 1px solid #FFCC80; padding: 12px; border-radius: 4px; color: #663C00; font-size: 13px; margin-bottom: 20px;">
           <strong>⚠️ Disclaimer:</strong> ${headerDisclaimer}
         </div>` : '';
...
<head>
  ...
  <style>
    ...
    h1, h2, h3 { color: #0070F2; }
    pre, code { background: #f5f5f5; padding: 2px 6px; border-radius: 4px; font-family: 'SF Mono', monospace; }
    pre { padding: 16px; overflow-x: auto; }
    blockquote { border-left: 3px solid #0070F2; margin: 16px 0; padding-left: 16px; color: #666; }
...
<body>
  ...
  <footer style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee; font-size: 12px; color: #999;">
    Exported from SAP Pro Toolkit on ${new Date().toLocaleString()}
  </footer>
</body>
</html>`;
    ...
    case 'md':
      processedContent = headerDisclaimer ? `> ⚠️ **Disclaimer:** ${headerDisclaimer}\n\n${content}` : content;
    ...
    default:
      ...
      processedContent = typeof stripMarkdown === 'function' ? stripMarkdown(content) : content;
      if (headerDisclaimer) {
        processedContent = `⚠️ Disclaimer: ${headerDisclaimer}\n\n${processedContent}`;
      }
  ...
  try {
    ...
    if (window.showToast) window.showToast(window.i18n('downloadedAsFormat', [extension.toUpperCase()]), 'success');
  } catch (error) {
    console.error('[Download] Failed:', error);
    if (window.showToast) window.showToast(window.i18n('downloadFailed'), 'error');
  }
};
...
window.showDiagnosticsModal = async function () {
  ...
  const downloadDropdown = document.getElementById('diagnosticsDownloadDropdown');
  ...
  contentDiv.innerHTML = `
    <div class="diagnostics-loading">
      ...
      <span>${window.i18n('aiAnalyzeClickToStart')}</span>
    </div>
  `;
};
...
window.regenerateDiagnosticsWithAI = async function () {
  ...
  if (!window.ToolkitCore || !window.ToolkitCore.testPromptWithModel) {
    if (window.showToast) {
      window.showToast(window.i18n('aiFeaturesRequireConfiguration'), 'warning');
      setTimeout(() => {
        if (window.showToast) window.showToast(window.i18n('configureAPIKeysInSettings'), 'info');
      }, 2000);
    }
    return;
  }

  // Check if on a SAP domain
  const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true });
  if (tab && tab.url) {
    const isSAPDomain = tab.url.match(/https:\/\/.*\.(sapsf|successfactors|hr\.cloud\.sap|s4hana|hana\.ondemand|build\.cloud\.sap|sap\.com)/);
    if (!isSAPDomain) {
      if (window.showToast) {
        window.showToast(window.i18n('aiDiagnosticsOnlySAPDomains'), 'info');
        setTimeout(() => {
          if (window.showToast) window.showToast(window.i18n('navigateToSAPPage'), 'info');
        }, 2000);
      }
      return;
    }
  ...
  if (regenerateBtn) {
    ...
    regenerateBtn.textContent = window.i18n('aiButtonAnalyzing');
  }
  ...
  contentDiv.innerHTML = `
    <div style="display: flex; align-items: center; gap: 12px; padding: 16px;">
      <div class="spinner"></div>
      <span>${window.i18n('aiAnalyzingPage')}</span>
    </div>
  `;
  ...
  try {
    const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true });
    if (!tab || !tab.url) {
      throw new Error(window.i18n('noActiveTab'));
    }
    ...
    if (!result || !result.content) {
      throw new Error(window.i18n('noResponseFromAI'));
    }
    ...
    // Use shared LLM response renderer for consistent display with badges (IDENTICAL to AI Notes format)
    contentDiv.innerHTML = window.renderLLMResponse({
      content: result.content,
      showBadges: true,
      model: result.model || 'Unknown',
      tokens: (result.usage?.inputTokens || 0) + (result.usage?.outputTokens || 0),
      disclaimer: window.i18n('aiDisclaimerDiagnostics')
    });
    ...
    if (window.showToast) window.showToast(window.i18n('aiDiagnosticsGenerated'), 'success');

  } catch (error) {
    console.error('[AI Diagnostics] Failed:', error);
    if (window.showToast) window.showToast(window.i18n('aiDiagnosticsFailed', [error.message]), 'error');
    contentDiv.innerHTML = `<p style="color: var(--env-production);">${window.i18n('failedGenerateDiagnosticsTryAgain')}</p>`;
...
window.saveDiagnosticsAsNote = async function () {
  ...
  if (!reportContent) {
    if (window.showToast) window.showToast(window.i18n('noReportContentToSave'), 'warning');
    return;
  }
  ...
  try {
    ...
    const noteTitle = window.i18n('diagnosticsReportTitle', [pageTitle, date]);
    ...
    window.renderNotes();
    if (window.showToast) window.showToast(window.i18n('diagnosticsReportSavedAsNote'), 'success');
  } catch (error) {
    console.error('[Save Diagnostics] Failed:', error);
    if (window.showToast) window.showToast(window.i18n('failedSaveReportAsNote'), 'error');
  }
};
...
window.downloadDiagnosticsReport = async function () {
  const modal = document.getElementById('diagnosticsModal');
  const reportContent = modal.getAttribute('data-ai-report');
  const pageTitle = modal.getAttribute('data-page-title') || window.i18n('diagnosticsReport');

  if (!reportContent) {
    if (window.showToast) window.showToast(window.i18n('noReportContentToDownload'), 'warning');
    return;
  }
  ...
  window.downloadFile({
    ...
    format: 'txt',
    title: pageTitle,
    headerDisclaimer: window.i18n('aiDisclaimerVerifyInformation')
  });
};
...
async function buildDiagnosticsPrompt(standardDiag, scrapedData, currentPageData) {
  ...
  try {
    ...
    throw new Error(window.i18n('failedLoadAIPromptConfiguration'));
  }
  ...
  const prompt = window.PromptLoader.fillPromptTemplate(promptTemplate, {
    ...
    testDataFound: testDataFound ? '⚠️ Test Data Found' : 'Clean',
  ...
  try {
    ...
    aiInsightsContent.innerHTML = `
      <div style="display: flex; align-items: center; gap: 12px; padding: 12px;">
        <span>${window.i18n('aiAnalyzingToolkitData')}</span>
      </div>
    `;
    ...
    if (window.showToast) window.showToast(window.i18n('aiSearchFailed', [error.message]), 'error');
...
function displayEnhancedAISearchResults(query, response, context) {
  ...
  // Build context header with query and stats
  const envCount = context.stats.totalEnvironments;
  const shortcutCount = context.stats.totalShortcuts;
  const noteCount = context.stats.totalNotes;
  const contextHeader = `
    <div class="ai-context-header">
      <div class="context-query">${window.i18n('aiContextQuery', [query])}</div>
      <div class="context-stats">${window.i18n('aiContextStats', [envCount, shortcutCount, noteCount])}</div>
    </div>
  `;

  // Use shared LLM response renderer for consistent display
  aiInsightsContent.innerHTML = `<div style="padding: 12px;">${window.renderLLMResponse({
    content: response,
    contextHeader: contextHeader,
    disclaimer: window.i18n('aiDisclaimerToolkit')
  })}</div>`;

  if (window.showToast) window.showToast(window.i18n('aiSearchComplete'), 'success');
}
...
window.handleRunAIPrompt = async function () {
  ...
  if (!content) {
    if (window.showToast) window.showToast(window.i18n('pleaseEnterPromptContentFirst'), 'warning');
    return;
  }

  if (!window.ToolkitCore || !window.ToolkitCore.testPromptWithModel) {
    console.error('[AI] ToolkitCore or testPromptWithModel not available');
    if (window.showToast) window.showToast(window.i18n('aiFeaturesNotAvailableToolkitCoreMissing'), 'error');
    return;
  }
  ...
  // Apply inline loading state with spinner (same pattern as AI Diagnostics and AI Search)
  textarea.style.background = 'rgba(16, 185, 129, 0.1)';
  textarea.style.borderColor = '#10B981';

  const originalContent = content;

  // Create loading container with spinner
  const loadingHTML = `<div style="display: flex; align-items: center; gap: 12px; padding: 16px; pointer-events: none;">
  <div class="spinner"></div>
  <span>${window.i18n('aiAnalyzingPrompt')}</span>
</div>`;
  ...
  // Insert loading indicator before textarea
  const loadingDiv = document.createElement('div');
  loadingDiv.id = 'noteContentLoading';
  ...
  try {
    ...
    try {
      ...
      if (window.showToast) window.showToast(window.i18n('noteRefinerTemplateMissingRunningRawPrompt'), 'warning');
    }
    ...
    if (!result) {
      if (window.showToast) window.showToast(window.i18n('noResponseFromAI'), 'warning');
      return;
    }
    ...
    if (window.showToast) window.showToast(window.i18n('aiTestFailed', [error.message]), 'error');
  } finally {
    // Remove loading indicator and restore textarea
    const loadingDiv = document.getElementById('noteContentLoading');
    if (loadingDiv) {
      loadingDiv.remove();
    }
...
window.loadLLMPricing = async function () {
  ...
  try {
    ...
    if (window.showToast) window.showToast(window.i18n('failedLoadPricingData'), 'error');
    return null;
  }
}
...
function showEstimateResults(result) {
  ...
  // Use shared LLM response renderer for consistent display (no reading time)
  contentEl.innerHTML = window.renderLLMResponse({
    content: result.responseContent,
    showBadges: true,
    model: result.modelData.model || result.modelId,
...
window.saveAIResponseAsNote = async function (fallbackResult) {
  ...
  if (!responseContent || !modelName) {
    if (fallbackResult && fallbackResult.responseContent) {
      ...
      if (window.showToast) window.showToast(window.i18n('noResponseContentToSave'), 'warning');
      return;
    }
  }
  ...
  try {
    ...
    const title = window.i18n('aiResponseNoteTitle', [dateStamp]);

    const cleanResponse = stripMarkdown(responseContent);

    let costLine = totalCost ? `${window.i18n('cost')}: $${totalCost}\n` : '';

    const content = `${cleanResponse}

---
${window.i18n('model')}: ${provider} - ${modelName}
${window.i18n('inputTokens')}: ${Number(inputTokens).toLocaleString()}
${window.i18n('outputTokens')}: ${Number(outputTokens).toLocaleString()}
${costLine}${window.i18n('generated')}: ${timestamp}`;

    // DUPLICATE CHECK: Prevent saving identical AI responses
    // Check for recent notes (within 5 minutes) with same title pattern and similar content
    const fiveMinutesAgo = Date.now() - (5 * 60 * 1000);
    const recentDuplicate = window.notes.find(note => {
      const isSameTitle = note.title === title;
      const isRecentlySaved = note.timestamp >= fiveMinutesAgo;
      const isSimilarContent = note.content && note.content.includes(cleanResponse.substring(0, 200)); // Check first 200 chars

      return isSameTitle && isRecentlySaved && isSimilarContent;
    });

    if (recentDuplicate) {
      if (window.showToast) {
        window.showToast(window.i18n('aiResponseAlreadySaved'), 'warning');
      }
      console.log('[Save AI Response] Duplicate detected, skipping save:', title);
      return;
    }
    ...
    if (window.showToast) window.showToast(window.i18n('aiResponseSavedAsNote'), 'success');

  } catch (error) {
    console.error('[Save AI Response] Failed:', error);
    if (window.showToast) window.showToast(window.i18n('failedToSave', [error.message]), 'error');
  }
};
...
window.copyAIResponseToClipboard = async function () {
  ...
  if (!responseContent) {
    if (window.showToast) window.showToast(window.i18n('noResponseContentToCopy'), 'warning');
    return;
  }
  ...
  try {
    ...
    if (window.showToast) window.showToast(window.i18n('responseCopiedToClipboard'), 'success');
  } catch (error) {
    console.error('[Copy AI Response] Failed:', error);
    if (window.showToast) window.showToast(window.i18n('failedCopyResponse'), 'error');
  }
};
...
window.openEnterpriseCalculator = function (testResult) {
  ...
  if (!modal || !contentEl) {
    ...
    if (window.showToast) window.showToast(window.i18n('enterpriseCalculatorNotAvailable'), 'error');
    return;
  }
  ...
  const html = `
    <div class="enterprise-calc-form">
      <h4 style="margin-bottom: 16px; color: var(--text-primary);">${window.i18n('scaleParameters')}</h4>
      
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
        <div class="form-group">
          <label>${window.i18n('numberOfUsers')}</label>
        ...
        <div class="form-group">
          <label>${window.i18n('queriesPerUserPerDay')}</label>
        ...
        <div class="form-group">
          <label>${window.i18n('workingDaysPerYear')}</label>
        ...
        <div class="form-group">
          <label>${window.i18n('expectedOutputTokens')}</label>
      ...
      <button class="btn btn-primary" id="calculateEnterpriseBtn" style="width: 100%; margin-bottom: 24px;">
        ${window.i18n('calculateAnnualProjections')}
      </button>
      
      <div id="enterpriseResults" style="display: none;">
        <h4 style="margin-bottom: 16px; color: var(--text-primary);">${window.i18n('annualCostProjection')}</h4>
  ...
  modal.setAttribute('data-test-input-tokens', testResult.usage?.inputTokens || 0);
...
function calculateEnterpriseProjections(testResult) {
  ...
  const outputTokens = parseInt(document.getElementById('enterpriseOutputTokens').value) || testResult.outputTokens || 0;

  const annualQueries = numUsers * queriesPerDay * workingDays;

  // Use tokens directly from testResult (not nested in usage)
  const inputTokensPerQuery = testResult.inputTokens || 0;
  const annualInputTokens = inputTokensPerQuery * annualQueries;
  const annualOutputTokens = outputTokens * annualQueries;

  // Calculate cost per query from costs object (already calculated in showEstimateResults)
        ...
        <div class="llm-response-content">
          ${originalPrompt ? `
            <div style="margin-bottom: 12px;">
              <strong style="font-size: 11px; color: var(--text-secondary); display: block; margin-bottom: 6px;">📝 ${window.i18n('originalPromptInput')}:</strong>
          ...
          ${aiResponse ? `
            <div>
              <strong style="font-size: 11px; color: var(--text-secondary); display: block; margin-bottom: 6px;">✨ ${window.i18n('aiResponseOutput')}:</strong>
  ...
  const html = `
    ...
    <div class="enterprise-projection-card" style="padding: 16px; background: var(--bg-secondary); border-radius: 6px; margin-bottom: 16px;">
      <h5 style="margin-bottom: 12px; color: var(--text-primary);">${window.i18n('volumeProjections')}</h5>
      <div class="metric-row">
        <span class="metric-label">${window.i18n('annualQueries')}:</span>
        <span class="metric-value">${annualQueries.toLocaleString()}</span>
      </div>
      <div class="metric-row">
        <span class="metric-label">${window.i18n('annualInputTokens')}:</span>
        <span class="metric-value">${annualInputTokens.toLocaleString()}</span>
      </div>
      <div class="metric-row">
        <span class="metric-label">${window.i18n('annualOutputTokens')}:</span>
    ...
    <div class="llm-response-card">
      <div class="llm-response-header">
        <span class="llm-response-title">
          ...
          ${window.i18n('costProjections')}
        </span>
      </div>
      <div class="llm-response-content">
      <div class="metric-row">
        <span class="metric-label">${window.i18n('costPerQuery')}:</span>
        <span class="metric-value">$${costPerQuery.toFixed(4)}</span>
      </div>
      <div class="metric-row">
        <span class="metric-label">${window.i18n('monthlyCost')}:</span>
        <span class="metric-value">$${monthlyCost.toFixed(2)}</span>
      </div>
        <div class="metric-row metric-total" style="font-size: 16px; font-weight: 700;">
          <span class="metric-label">${window.i18n('annualCost')}:</span>
    ...
    <div style="margin-top: 16px; padding: 12px; background: var(--bg-tertiary); border-radius: 6px; font-size: 11px; color: var(--text-secondary);">
      <strong>${window.i18n('assumptions')}:</strong> ${numUsers.toLocaleString()} users × ${queriesPerDay} queries/day × ${workingDays} working days
    </div>
  ...
  // Show download dropdown and wire up format selection
  const downloadDropdown = document.getElementById('enterpriseReportDownloadDropdown');
  if (downloadDropdown) {
  ...
  // Wire up download format buttons
  document.querySelectorAll('#enterpriseReportDownloadMenu .dropdown-item').forEach(btn => {
  ...
  document.querySelectorAll('#enterpriseReportDownloadMenu .dropdown-item').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      exportEnterpriseReport(reportData, format);
    });
  });

  if (window.showToast) window.showToast(window.i18n('projectionsCalculated'), 'success');
};

function exportEnterpriseReport(data, format = 'txt') {
  ...
  const reportContent = `SAP Pro Toolkit - Enterprise AI Cost Projection
...
`;
  ...
  // Use shared download utility for consistent format handling
  window.downloadFile({
    content: reportContent,
    filename: `enterprise-ai-cost-projection-${timestamp}`,
    format: format,
    title: 'Enterprise AI Cost Projection',
    headerDisclaimer: window.i18n('aiDisclaimerVerifyInformation')
  });
}
...
window.addShortcutWithAI = async function () {
  if (!window.ToolkitCore || !window.ToolkitCore.testPromptWithModel) {
    if (window.showToast) window.showToast(window.i18n('aiFeaturesNotAvailable'), 'error');
    return;
  }
  ...
  try {
    ...
    notesField.value = window.i18n('aiAnalyzingCurrentPage');
    ...
    if (!tab || !tab.url) {
      ...
      if (window.showToast) window.showToast(window.i18n('noActiveTabFound'), 'warning');
      return;
    }
    ...
    if (tab.url.startsWith('chrome://') || tab.url.startsWith('edge://') || tab.url.startsWith('about:')) {
      ...
      notesField.style.background = '';
      notesField.style.borderColor = '';
      if (window.showToast) window.showToast(window.i18n('cannotScrapeBrowserInternalPages'), 'warning');
      document.getElementById('shortcutName').value = tab.title;
      document.getElementById('shortcutPath').value = tab.url;
      return;
    }

    notesField.value = window.i18n('aiPreparingToScrapePage');
    try {
      await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        files: ['content/content.js']
    ...
    notesField.value = window.i18n('aiScrapingPageContent');
    const scrapedData = await new Promise((resolve) => {
      chrome.tabs.sendMessage(tab.id, { action: 'scrapePageForShortcut' }, (response) => {
        if (chrome.runtime.lastError) {
          ...
          resolve({ error: 'Failed to scrape page', title: tab.title, url: tab.url, content: '', metaDescription: '' });
        } else {
          resolve(response);
        }
      });
    ...
    const isSAPDomain = tab.url.match(/https:\/\/.*\.(sapsf|successfactors|hr\.cloud\.sap|s4hana|hana\.ondemand|build\.cloud\.sap|sap\.com)/);
    if (!isSAPDomain) {
      // Non-SAP domain: Use basic extraction with meta description (no AI required)
      console.log('[AI Shortcut] Non-SAP domain detected, using fallback extraction');
      ...
      // Build structured summary WITHOUT AI
      let summary = `${scrapedData.title}\n\n`;
      summary += `URL: ${scrapedData.url}\n`;
      ...
      document.getElementById('shortcutName').value = scrapedData.title;
      document.getElementById('shortcutIcon').value = window.DEFAULT_ICONS.shortcut;
      ...
      // ✅ NO toast notifications (per user requirement: "keep toast notifications minimum")
      console.log('[AI Shortcut] Non-SAP shortcut populated with basic extraction');
      
      return;
    }

    // SAP domain: Continue with AI enhancement
    notesField.value = window.i18n('aiEnhancingTitle');
    ...
    notesField.value = window.i18n('aiGeneratingSummary');
    ...
    if (window.showToast) window.showToast(window.i18n('aiSummaryGenerated'), 'success');
    ...
    notesField.style.background = '';
    notesField.style.borderColor = '';
    if (window.showToast) window.showToast(window.i18n('aiShortcutCreationFailed', [error.message]), 'error');
  } finally {
