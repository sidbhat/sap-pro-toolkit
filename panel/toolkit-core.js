  ...
  try {
    ...
    if (orchestrationResponse.ok) {
      ...
      return {
        content,
        model: modelName,
        provider: 'SAP AI Core (Orchestration)',
        usage
      };
    } else {
      console.warn('[SAP AI Core] Orchestration failed:', orchestrationResponse.status);
    }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (errorText.includes('unauthorized') || errorText.includes('forbidden')) {
        throw new Error(`Authentication failed. Check: Settings → SAP AI Core → Verify credentials`);
      }
    }
  } catch (specificError) {
  ...
  try {
    ...
    if (fallbackResponse.ok) {
      ...
      return {
        ...
        provider: 'SAP AI Core',
        usage: {
          inputTokens: fallbackData.usage?.input_tokens || 0,
          outputTokens: fallbackData.usage?.output_tokens || 0,
          cost: 'N/A (SAP AI Core)'
        }
      };
    }
